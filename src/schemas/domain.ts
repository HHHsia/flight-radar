...
/** IATA airport / city code, or comma-separated list (e.g. NRT,HND for SerpApi `arrival_id`). */
const serpApiLocationIdSchema = z.string().min(3).max(64).regex(/^[A-Za-z0-9/]+(,[A-Za-z0-9/]+)*$/);

export const trackedDestinationSchema = z.object({
  id: z.string().min(1),
  originAirportCode: z.string().length(3),
  destinationAirportCode: serpApiLocationIdSchema,
